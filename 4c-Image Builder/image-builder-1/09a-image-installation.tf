...
resource "aws_snapshot" "snapshot-from-ami-1" {
  count       = length(aws_ami.ami_from_imagebuilder-1[count.index].block_device_mappings)
  snapshot_id = aws_ami.ami_from_imagebuilder-1[count.index].block_device_mappings[count.index].snapshot_id
