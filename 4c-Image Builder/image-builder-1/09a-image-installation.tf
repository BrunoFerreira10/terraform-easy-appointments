...
resource "aws_ami" "ami-from-imagebuilder-1" {
  ...
  id   = element(aws_imagebuilder_image.image-1.output_resources[0].amis, count.index).image_id
...
resource "aws_snapshot" "snapshot-from-ami-1" {
  count       = length(aws_ami.ami-from-imagebuilder-1[count.index].block_device_mappings)
  snapshot_id = aws_ami.ami-from-imagebuilder-1[count.index].block_device_mappings[count.index].snapshot_id
